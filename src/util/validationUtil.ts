...
export const validateCredentials = (email: string, token: string): void => {
    ...
    const formattedDummyDate: string = new Date().toLocaleDateString("es-ES", {
        timeZone: "UTC",
    });
    ...
    fetchBCCRWebService(
        ...
        .then((txt) => {
            try {
                ...
                throw new Error(
                    "Bad credentials. Email or token (or both) rejected by BCCR web service.",
                );
            }
        });
};
 ...
 * @param token Access token given by BCCR for web service.
 */
export const validateConstructorParameters = (
    email: string,
    token: string,
): void => {
    if (!email.length) {
        throw new Error(
            "Found empty string as email constructor parameter. This parameter must be an string containing a valid email address.",
        );
    }
    if (!token.length) {
        throw new Error(
            "Found empty string as token constructor parameter. This parameter must be an string containing an alphanumerical value.",
        );
    }
};
...
export const validateRequestParameters = (
    ...
    if (!code.length) {
        throw new Error(
            "Found empty string as indicator code parameter. This parameter must be an string containing a numerical value.",
        );
    }
